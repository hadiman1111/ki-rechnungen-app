...
def _prefer_env_flet_view_path() -> None:
    """Prefer ``FLET_VIEW_PATH`` over cwd ``build/macos`` (Flet default order).

    Flet desktop resolves macOS clients as: build/macos → FLET_VIEW_PATH → cache.
    Leftover ``build/macos/*.app`` often still carries the generic Flet/fish icon.
    The Dock-App bundles a branded client and sets ``FLET_VIEW_PATH``; this patch
    makes that client win so the Dock shows KI-Rechnungen instead of the fish.
    """
